...
//   removeFromCart: (itemId: number) => Promise<void>;
//   updateQuantity: (itemId: number, quantity: number) => Promise<void>;
//   clearCart: () => Promise<boolean>; // Updated return type to Promise<boolean>
//   cartQuantity: number;
...
//   // Add to cart function
//   const addToCart = async (item: CartItem) => {
//     setLoadingItems((prev) => new Set(prev).add(item.id)); // Should add item.id to loadingItems
//     try {
//         let updatedCart = [...cart];
//         const existingItemIndex = updatedCart.findIndex((cartItem) => cartItem.id === item.id);
//         if (existingItemIndex !== -1) {
...
//   // Clear cart function with loading state and return value
//   const clearCart = async (): Promise<boolean> => {
//     setLoadingItems((prev) => new Set(prev).add(-1)); // Using -1 as a special flag
//     try {
//       setCart([]);
