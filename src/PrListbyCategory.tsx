...
//         console.log('Category Result:', categoryResult); // Add this line
//         setCategoryName(categoryResult.category.title); // Set the category name
...
//         <Text style={styles.title}>{categoryName}</Text>
//         <TouchableOpacity onPress={() => navigation.navigate("ProductListScreen", { categoryId })}>
  ...
  useEffect(() => {
    const fetchCategoryAndProducts = async () => {
      try {
        ...
        const categoryResult = await categoryResponse.json();
        console.log('Category Result:', categoryResult); // Log the response
  
        if (categoryResult && categoryResult.category.title) {
          setCategoryName(categoryResult.category.title); // Set the category name
        } else {
          console.error('Category name not found in response:', categoryResult);
          setCategoryName('Unknown Category'); // Fallback name
        }
  
        // Fetch all products
        const productsResponse = await fetch(
          `https://api.g3studio.co/api/products`,
          requestOptions
        );
        const productsResult = await productsResponse.json();
